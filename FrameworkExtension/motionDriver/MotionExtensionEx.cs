...
namespace Automation.FrameworkExtension.motionDriver
{
    ...
    public static partial class MotionExtensionEx
    {
        ...
        {
            int sts;
            vioex.Driver.GetDo(vioex.Port, out sts);
            return (sts == 1) == status;
        }
    ...
    public static partial class MotionExtensionEx
    {
        public static void SetDoAsync(this ICylinderEx[] cyex, StationTask task, bool status = true)
        {
            ...
            {
                if (cyex[i].DoOrg >= 0)
                    cyex[i].Driver2.SetDo(cyex[i].DoOrg, status ? 0 : 1);
                if (cyex[i].DoWork >= 0)
                    cyex[i].Driver2.SetDo(cyex[i].DoWork, status ? 1 : 0);
            }
        }
        ...
        public static bool SetDo(this ICylinderEx[] cyex, StationTask task, bool[] status, int timeout = 3000, bool? ignoreOrWaringOrError = false)
        {
            ...
            {
                if (cyex[i].DoOrg >= 0)
                    cyex[i].Driver2.SetDo(cyex[i].DoOrg, status[i] ? 0 : 1);
                if (cyex[i].DoWork >= 0)
        ...
        {
            return WaitDi(new[] { cyex.DiOrg, cyex.DiWork }, task, new[] { cyex.Driver1, cyex.Driver1 }, new[] { !status, status }, timeout);
        }
        ...
        public static bool WaitDi(this ICylinderEx[] cyex, StationTask task, bool[] status, int timeout = -1, bool? ignoreOrWaringOrError = false)
        {
            ...
            var motion1 = cyex.Select(c => c.Driver1);
            var motion2 = cyex.Select(c => c.Driver1);
    ...
    public static partial class MotionExtensionEx
    {
        ...
        {
            int sts;
            doex.Driver.GetDo(doex.Port, out sts);
        ...
        {
            doex.Driver.SetDo(doex.Port, status ? 1 : 0);
            return true;
        }
        ...
        public static bool SetDoAsync(this IDoEx[] doex, bool[] status)
        {
            ...
            {
                doex[i].Driver.SetDo(doex[i].Port, status[i] ? 1 : 0);
            }
        ...
        {
            return WaitDo(doex.Select(d => d.Port).ToArray(), task, doex.Select(d => d.Driver).ToArray(), status, timeout);
        }
        ...
        public static bool GetDiSts(this IDiEx diex, bool status = true)
        {
            if (FrameworkManager.Ins.IsSimulate)
            {
                ...
                else if (diex.Type == DiType.NormalClose)
                {
                    return diex.Enable;
                }
                else
            ...
            {
                int sts;
                diex.Driver.GetDi(diex.Port, out sts);
        ...
        {
            return WaitDi(diex.Select(d => d.Port).ToArray(), task, diex.Select(d => d.Driver).ToArray(), diex.Select(d => status).ToArray(), timeout);
        }


        public static bool WaitDi(this IDiEx diex, StationTask task, bool status = true, int timeout = -1)
        {
            return WaitDi(diex.Port, task, diex.Driver, status, timeout);
        }
    ...
    public static partial class MotionExtensionEx
    {
        public static bool Stop(this IAxisEx[] axis)
        {
            ...
            {
                a?.Driver.MoveStop(a.AxisChannel);
            }
        ...
        {
            return a.Driver.GetAxisAlarm(a.AxisChannel) || a.Driver.GetAxisEmg(a.AxisChannel);
        }

        public static bool GetServo(this IAxisEx a)
        {
            return a.Driver.GetAxisEnable(a.AxisChannel);
        }

        public static bool GetMdn(this IAxisEx a)
        {
            return a.Driver.GetAxisDone(a.AxisChannel);
        }

        public static bool GetAstp(this IAxisEx a)
        {
            return a.Driver.GetAxisAstp(a.AxisChannel);
        }

        public static bool GetInp(this IAxisEx a)
        {
            return a.Driver.GetAxisInp(a.AxisChannel);
        }

        public static bool GetPel(this IAxisEx a)
        {
            return a.Driver.LimitPel(a.AxisChannel);
        }

        public static bool GetOrg(this IAxisEx a)
        {
            return a.Driver.LimitOrg(a.AxisChannel);
        }

        public static bool GetMel(this IAxisEx a)
        {
            return a.Driver.LimitMel(a.AxisChannel);
        }
        ...
        {
            int pos = 0;
            axis?.Driver.GetEncPos(axis.AxisChannel, ref pos);
            return pos;
        }
        ...
        public static bool ServoEnable(this IAxisEx[] axis, StationTask task, bool status)
        {
            ...
            foreach (var a in axis)
            {
                a?.Driver.ServoEnable(a.AxisChannel, status);
            }
        ...
        public static bool Home(this IAxisEx[] axis, StationTask task, double[] vel, int timeout = -1)
        {
            ...
            foreach (var a in axis)
            {
                a?.Driver.Home(a.AxisChannel, a.ToPls(a.HomeVm));
            }
            ...
            while (t++ <= timeout)
            {
                if (axis.All(a =>
                    a == null
                    || (axisInps[Array.IndexOf(axis, a)] && a.GetInp() && a.Driver.CheckHomeDone(a.AxisChannel))
                    || (!axisInps[Array.IndexOf(axis, a)] && !a.GetInp() && a.Driver.CheckHomeDone(a.AxisChannel)))
                )
                {
                    var ret = axis.All(a => a.Driver.SetEncPos(a.AxisChannel, 0) == 0);
                    task?.Log($"{err} success {ret}", LogLevel.Debug);
                    return true;
                }

                if (axis.Any(a => a != null && (a.GetAlarm() || a.GetAstp())) || axis.All(a => a == null || (a.GetMdn() && !a.Driver.CheckHomeDone(a.AxisChannel))))
                {
                    axis.Stop();
        ...
        public static bool MoveAbsAsync(this IAxisEx[] axis, StationTask task, double[] pos, double[] vel)
        {
            ...
            for (int i = 0; i < axis.Length; i++)
            {
                ...
                axis[i]?.Driver.MoveAbs(axis[i].AxisChannel, axis[i].ToPls(pos[i]), axis[i].ToPls(vel[i]));
            }
        ...
        public static bool MoveRel(this IAxisEx[] axis, StationTask task, double[] step, double[] vel, int timeout = -1, bool isCheckLimit = true)
        {
            ...
            for (int i = 0; i < axis.Length; i++)
            {
                if (axis[i] != null)
                    axis[i].Error = string.Empty;
                axis[i]?.Driver.GetEncPos(axis[i].AxisChannel, ref startpls[i]);
                axis[i]?.Driver.MoveRel(axis[i].AxisChannel, axis[i].ToPls(step[i]), axis[i].ToPls(vel[i]));
            }
            ...
            while (t++ <= timeout)
            {
                ...
                if (task != null)
                {
                    ...
                    if (task.RunningState == RunningState.Pause)
                    {
                        ...
                        for (int i = 0; i < axis.Length; i++)
                        {
                            axis[i]?.Driver.GetEncPos(axis[i].AxisChannel, ref pausepls[i]);
                        }
                        ...
                        for (int i = 0; i < axis.Length; i++)
                        {
                            axis[i]?.Driver.MoveRel(axis[i].AxisChannel, axis[i].ToPls(step[i]) - (pausepls[i] - startpls[i]), axis[i].ToPls(vel[i]));
                        }
                    }
                }
                else
        ...
        private static bool IsMoveErrorHappen(IAxisEx axis)
        {
            ...
            Thread.Sleep(1);
            var p1 = 0;
            axis.Driver.GetCommandPos(axis.AxisChannel, ref p1);
            var p2 = 0;
            axis.Driver.GetEncPos(axis.AxisChannel, ref p2);
