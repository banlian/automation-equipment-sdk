...
namespace Automation.FrameworkExtension.userControls
{
    public partial class CylinderControl : UserControl, IFrameworkControl
    {
        ...
        public List<CylinderEx> CyExs;

        public void LoadFramework()
        {
            var props = typeof(CylinderEx).GetProperties().Where(p => p.PropertyType != typeof(MotionCardWrapper)).ToArray();
            ...
            if (CyExs != null)
            {
                foreach (var ex in CyExs)
                {
                    ...
                    {
                        dataGridView1.Rows[r].Cells[i].Value = props[i].GetValue(ex);
                    }
                }
            }
        }
